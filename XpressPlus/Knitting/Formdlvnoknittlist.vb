Public Class Formdlvnoknittlist
    ...
    Private Sub Bindingmaster()
        ...
        'Tmaster = SQLCommand("SELECT '' AS Stat,Dlvno FROM Tdeliyarnxp
        '                        WHERE Comid = '" & Gscomid & "' AND Dlvno NOT IN 
        '                        (SELECT Dlvno FROM Tknittcomxp WHERE Comid = '" & Gscomid & "')")
        Tmaster = SQLCommand("SELECT '' AS Stat,Dlvno ,Dyarndate FROM Tdeliyarnxp
                                WHERE Comid = '" & Gscomid & "' AND Dlvno NOT IN 
                                (SELECT Dlvno FROM Tknittcomxp WHERE Comid = '" & Gscomid & "')")
    ...
    Private Sub Filtermastergrid()
        ...
        If Tbkeyword.Text = "" Then
            ...
            Exit Sub
        End If
        Tmaster.DefaultView.RowFilter = String.Format("Dlvno Like '%{0}%'", Trim(Tbkeyword.Text))
    End Sub
